--------------------------------------------------------------------------------
-- 81-718 pneumatic system
--------------------------------------------------------------------------------
-- Copyright (C) 2013-2018 Metrostroi Team & FoxWorks Aerospace s.r.o.
-- Contains proprietary code. See license.txt for additional information.
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_718_Pneumatic")
...
function TRAIN_SYSTEM:Initialize(parameters)
    ...
    -- Вентили дверного воздухораспределителя (ВДЗ, ВДОЛ, ВДОП)
    self.Train:LoadSystem("U1","Relay","", {bass = true})
    self.Train:LoadSystem("U2","Relay","", {bass = true})
    self.Train:LoadSystem("U3","Relay","", {bass = true})


    self.Train:LoadSystem("EPK","Relay","Switch",{ bass = true})
    --self.Train:LoadSystem("EPKContacts","Relay","",{close_time = 3})
    --Вентиль ЭПВ
    self.Train:LoadSystem("U4","Relay","")
    --Вентиль управления подъёмом ТП
    self.Train:LoadSystem("U5","Relay","")
    -- Valve #2
    self.Train:LoadSystem("U6","Relay")
    -- Valve #1
    self.Train:LoadSystem("U7","Relay")

    -- Регулятор давления (АК)
    self.Train:LoadSystem("SP1","Relay","AK-11B")
    -- Автоматический выключатель управления (АВУ)
    self.Train:LoadSystem("SP2","Relay","AVU-045")
    -- Автоматический выключатель торможения (АВТ)
    self.Train:LoadSystem("SP3","Relay","AVT-325")
    --ДКПТ
    self.Train:LoadSystem("SP4","Relay")
    --СОТ1 ЭПВ Осн
    self.Train:LoadSystem("SP5","Relay")
    --СОТ2 ЭПВ Рез
    self.Train:LoadSystem("SP6","Relay")
    --СОТ3
    self.Train:LoadSystem("SP7","Relay")
    ---- Блокировка тормозов
    --self.Train:LoadSystem("BPT","Relay","")
    -- SA15-22 Концевые выключатели дверей
    self.Train:LoadSystem("SAD","Relay","")
    -- SQ1(2) - Датчики томроза
    self.Train:LoadSystem("SQ1","Relay","")
    --Концевой датчик
    self.Train:LoadSystem("SQ3","Relay","")


    if self.DisconnectType then
        -- Краны двойной тяги
...
function TRAIN_SYSTEM:TriggerInput(name,value)
    ...
    elseif name == "Autostop" then
        ...
        if HaveUAVA and self.Train.UAVA.Value == 0 then
            ...
            --self.Train.ALS_ARS.UAVAContacts = true
...
function TRAIN_SYSTEM:Think(dT)
    ...
    local BLDisconnect = self.DisconnectType and Train.DriverValveBLDisconnect.Value > 0 or Train.DriverValveDisconnect.Value > 0
    ...
    if Train.U4 then
        ...
        if Train.EPK.Value == 0 or Train.U4.Value == 0 or self.ValveType==2 and not BLDisconnect then
    ...
    if Train.AirDistributorDisconnect.Value == 0 then
        ...
        if (Train.U7.Value == 1.0) and (Train.U6.Value == 0.0) then
            if self.PN1 == 0 then
                --1,2
        ...
        -- Valve #2
        if Train.U6.Value == 1.0 then
            if self.PN2 == 0 then
                self.PN2 = math.min(self.TrainLinePressure,(2.7 + error + self.WeightLoadRatio*1.2))
    ...
    self.Compressor = Train.KK.Value * (Train.Electric.Aux750V and 1 or 0)
    ...
    Train.SP3:TriggerInput("Open", self.BrakeCylinderPressure > 1.9) -- 1.8 - 2.0
    Train.SP3:TriggerInput("Close",self.BrakeCylinderPressure < 1.2) -- 0.9 - 1.5
    Train.SP1:TriggerInput( "Open", self.TrainLinePressure > 8.2)
    Train.SP1:TriggerInput( "Close",self.TrainLinePressure < 6.3)
    ...
    if self.DoorLinePressure > 3.5 then
        if (Train.U2.Value == 1.0) and (Train.U3.Value == 0.0) and not self.DoorLeft then
            self.DoorLeft = true
        end
        if (Train.U2.Value == 0.0) and (Train.U3.Value == 1.0) and not self.DoorRight then
            self.DoorRight = true
        end
        if (Train.U1.Value == 1.0 or Train.U2.Value == 1.0 and Train.U3.Value == 1.0 or self.RZDTimer) and (self.DoorLeft or self.DoorRight) then
        ...
        if Train.U2.Value == 1.0 and Train.U3.Value == 1.0 then
    ...
    Train.SAD:TriggerInput("Set",not Train.RightDoorsOpen and not Train.LeftDoorsOpen)
